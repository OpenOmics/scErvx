...
def setup(sub_args, ifiles, repo_path, output_path):
    ...
    if sub_args.mode =='slurm':
        genome_config = os.path.join(output_path, 'config', sub_args.genome + '_biowulf.json')
    elif sub_args.mode =='uge':
        genome_config = os.path.join(output_path, 'config', sub_args.genome + '_locus.json')
    else:
        genome_config = os.path.join(output_path, 'config', sub_args.genome + '_biowulf.json')
...
def add_sample_metadata(input_files, config, group=None):
    ...
    for file in input_files:
        ...
        sample = re.split('\.bam$|\.R[12]\.fastq\.gz', os.path.basename(file))[0]
...
def runner(mode, outdir, alt_cache, logger, additional_bind_paths = None, 
    threads=2,  jobname='pl_master', submission_script='run.sh',
